...
const Emoji = ({ name, value, size, handleClick, icon }) => {
  return (
    <div className='h-8 w-8'>
      <input name={name} type="radio" className={`absolute-h-${size} w-${size} appearance-none`} value={value} onClick={event => handleClick(event)} />
      <svg aria-hidden="true" focusable="false" data-prefix="far" data-icon={icon} class="w-5 text-blue-500 mr-1" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 496 512">
